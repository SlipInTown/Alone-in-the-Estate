...
public static class PoolManager
{
    ...
    private static GameObject objectParent;
    ...
    public static void Initialize(PoolPart[] newPools)
    {
        ...
        objectParent.name = "Pool"; //создаем на сцене объект Pool, чтобы не заслонять иерархию
        for (int i = 0; i < pools.Length; i++)
        {
            if (pools[i].prefab != null)
            {
                pools[i].ferula = new ObjectPooling(); //Создаем пул для каждого префаба
                pools[i].ferula.Initialize(pools[i].count, pools[i].prefab, objectParent.transform); //инициализируем пул заданным количеством объектов
            }
        }
    }
        ...
        if (pools != null)
        {
            for (int i = 0; i < pools.Length; i++)
            {
                if (string.Compare(pools[i].name, name) == 0) //если имя совпало с именем префаба пула
                {
                    ...
                    result.SetActive(true); //выставляем координаты, вращение и активируем
                    return result;
                }
            }
        }
        return result; //Если на нашлось такого объекта в пуле, то вернет null
    }
}
